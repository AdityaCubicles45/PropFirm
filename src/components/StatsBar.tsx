...
interface StatsBarProps {
  ...
  currentPrice: number | null; // This will now be updated by the OrderBook via page.tsx
}

const StatItem: React.FC<{ label: string; value: string | number | undefined; positive?: boolean; negative?: boolean, unit?: string }> = ({ label, value, positive, negative, unit }) => (
...
export default function StatsBar({ selectedToken, onTokenSelect, currentPrice }: StatsBarProps) {
  const indexPrice = currentPrice ? (currentPrice * 0.998).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 5 }) : 'N/A';
